...
def generate_structure(vae, z, s_cond=None, s_tensor_cond=None):
    ...
    for track_idx in range(s_tensor.size(0)):
        ...
        track_structure = []
        for idx in range(s_tensor.size(-1)):
            
            print(f"Processing phrase {idx} in track {track_idx}")
            
            print(f"node_idx: {node_idx}")
            
            if s_tensor[track_idx, :, :, idx].sum() == 0: # If there's no activation in Mel/Non-Mel
                # Skip this note
            ...
            phrase_ids, phrase_lens = torch.argmax(phrase_ids), torch.argmax(phrase_lens)
...
#                 break
            ...
            if (phrase_ids == IdToken.SOS.value or
                    phrase_ids == IdToken.SOS.value):
                # Skip this note
            ...
            track_structure.append({"phrase_id": phrase_ids, 
                              "length": phrase_lens, 
