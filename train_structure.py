...
from data_structure import PolyphemusDataset
import torch.optim as optim

from model_structure import VAE
from utils import set_seed, print_params, print_divider
from training_structure import PolyphemusTrainer, ExpDecayLRScheduler, StepBetaScheduler


def main():
    parser = argparse.ArgumentParser(description="Trains Polyphemus.")
    parser.add_argument(
        "dataset_dir",
        type=str,
    ...
    parser.add_argument(
        "output_dir", type=str, help="Directory to save the output of the training."
    )
    parser.add_argument(
        "config_file", type=str, help="Path to the JSON training configuration file."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default="10",
        help="The number of processes to use for loading the data. " "Default is 10.",
    )
    parser.add_argument(
        "--tr_split",
        type=float,
    ...
    trainloader = DataLoader(
        tr_set, batch_size=batch_size, shuffle=True, num_workers=args.num_workers
    )
    if args.eval:
        validloader = DataLoader(
            vl_set, batch_size=batch_size, shuffle=False, num_workers=args.num_workers
        )
        testloader = DataLoader(
            ts_set, batch_size=batch_size, shuffle=False, num_workers=args.num_workers
        )
    ...
    model_name = args.model_name if args.model_name is not None else str(uuid.uuid1())
    ...
    # Create the model
    print(f"Creating the model and moving it on {device} device...")
    ...
    lr_scheduler = ExpDecayLRScheduler(
        optimizer=optimizer, **training_config["lr_scheduler"]
    )
