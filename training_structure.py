...
class PolyphemusTrainer():
    ...
    def __init__(self, model_dir, model, optimizer, init_lr=1e-4,
        ...
        self.ce_p = nn.CrossEntropyLoss(ignore_index=IdToken.PAD.value)
        self.ce_d = nn.CrossEntropyLoss(ignore_index=LengthToken.PAD.value)
    ...
    def _losses(self, s_tensor, s_logits, c_tensor, c_logits, mu, log_var):
        ...
        pitch_logits = c_logits[:, :constants.N_ID_TOKENS]
        pitch_true = c_tensor[:, :constants.N_ID_TOKENS].argmax(dim=1)
        pitch_loss = self.ce_p(pitch_logits, pitch_true)

        # Content tensor loss (durations)
        dur_logits = c_logits[:, constants.N_ID_TOKENS:]
        dur_true = c_tensor[:, constants.N_ID_TOKENS:].argmax(dim=1)
    ...
    def _duration_accuracy(self, c_logits, c_tensor):
        ...
        dur_rec = c_logits[..., constants.N_ID_TOKENS:]
        dur_rec = F.softmax(dur_rec, dim=-1)
        dur_rec = torch.argmax(dur_rec, dim=-1)

        dur_true = c_tensor[..., constants.N_ID_TOKENS:]
        dur_true = torch.argmax(dur_true, dim=-1)

        # Do not consider PAD tokens when computing accuracies
        not_pad = (dur_true != LengthToken.PAD.value)
    ...
    def _note_accuracy(self, c_logits, c_tensor):
        ...
        not_pad_p = (pitch_true != IdToken.PAD.value)
        ...
        not_pad_d = (dur_true != LengthToken.PAD.value)
